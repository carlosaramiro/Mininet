...
class Mininet( object ):
    ...
    def iperf( self, hosts=None, l4Type='TCP', udpBw='10M', fmt=None,
	    ...
	    print len( re.findall( '/sec', servout ) )
        server.sendInt()
    ...
    def debiperf( self, hosts=None, l4Type='TCP', udpBw='10M', fmt=None,
               seconds=5, port=5001):
	print port
	print time.time()
        """Run iperf between two hosts.
        ...
        servout += server.waitOutput()
        debug( 'Server output: %s\n' % servout )
        result = [ self._parseIperf( servout ), self._parseIperf( cliout ), time.time() ]
        if l4Type == 'UDP':
            result.insert( 0, udpBw )
        output( '*** Results: %s\n' % result )
        return result
    ...
    def myiperf( self, hosts=None, iterations=10,
               seconds=1, port=5001):
	client=[]
	server=[]
	a=server
	...
	for l in range (iterations):
	    ...
	    for i in range (len(client)):
	        ...
	        output( '*** Iperf: testing', 'bandwidth between',
                ...
                cliout = client[i].cmd( 'iperf -p %d ' % b[x] + ' -t %d -c ' % seconds +
                                 a[x].IP() + ' ' + bwArgs )
